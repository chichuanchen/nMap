...
```
## Calculate avgerage number of trial per subject
```{r}
n_trial_subj %>% group_by(uID) %>% summarise(total_per_subj = sum(n_trial_individual)) %>% summarise(mean_trial=mean(total_per_subj)) %>% as.data.frame()
```
